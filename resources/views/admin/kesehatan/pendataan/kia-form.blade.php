...
<div class="mb-6 flex items-center gap-3">
    ...
    <h3 class="text-base font-semibold text-gray-900">{{ isset($kia) ? 'Edit KIA: '.$kia->nama_ibu : 'Tambah Data KIA'
        }}</h3>
</div>
...
<form method="POST"
    ...
    <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="posyandu_id"
                        ...
                        <option value="{{ $p->id }}" {{ old('posyandu_id', $kia->posyandu_id ?? '') == $p->id ?
                            'selected' : '' }}>{{ $p->nama_posyandu }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="tempat_pemeriksaan"
                        ...
                        @foreach(['Posyandu','Puskesmas','Klinik','Rumah Sakit','Bidan Praktek','Dokter Praktek'] as $t)
        ...
        {{-- Data Ibu --}}
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-5">Data Ibu</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                <div class="lg:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Ibu <span
                            class="text-red-500">*</span></label>
                    <input type="text" name="nama_ibu" required value="{{ old('nama_ibu', $kia->nama_ibu ?? '') }}"
                        class="w-full px-4 py-2.5 text-sm border @error('nama_ibu') border-red-400 @else border-gray-200 @enderror rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                ...
                <div>
                    ...
                    <input type="text" name="nik_ibu" maxlength="20" value="{{ old('nik_ibu', $kia->nik_ibu ?? '') }}"
                ...
                <div>
                    ...
                    <input type="text" name="no_hp" value="{{ old('no_hp', $kia->no_hp ?? '') }}" placeholder="08xxx"
                        class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                </div>
                <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div>
                        ...
                        <select name="jenis_persalinan"
                            class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                            <option value="">-- Pilih --</option>
                            @foreach(['normal'=>'Normal','sesar'=>'Sesar','vakum'=>'Vakum'] as $val => $lbl)
                    ...
                    <div>
                        ...
                        <select name="penolong_persalinan"
                            class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                            <option value="">-- Pilih --</option>
                            @foreach(['Dokter','Bidan','Dukun Terlatih','Dukun Tidak Terlatih','Lainnya'] as $p)
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            ...
            <p class="text-xs text-gray-400 mb-5">Isi setelah anak lahir</p>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                <div class="lg:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Anak</label>
                    <input type="text" name="nama_anak" value="{{ old('nama_anak', $kia->nama_anak ?? '') }}"
                        class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">NIK Anak</label>
                    <input type="text" name="nik_anak" value="{{ old('nik_anak', $kia->nik_anak ?? '') }}"
                        class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                </div>
                <div>
                ...
                <div class="lg:col-span-3">
                    ...
                    <textarea name="keterangan" rows="2"
                        class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition resize-none">{{ old('keterangan', $kia->keterangan ?? '') }}</textarea>
                </div>
            </div>
        </div>
...
document.getElementById('status_kehamilan')?.addEventListener('change', function () {
    const el = document.getElementById('bagian-melahirkan');
    el.classList.toggle('hidden', this.value === 'hamil');
});
</script>
@endsection