...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Edit Barang Inventaris</h1>
                <p class="text-sm text-slate-500 mt-1">Perbarui informasi barang inventaris</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 px-6 py-4 rounded-xl shadow-sm">
        <div class="flex items-start gap-3">
            ...
            <div>
                <p class="text-sm font-semibold text-blue-900">Barang: {{ $inventaris->nama_barang }}</p>
                <p class="text-xs text-blue-700 mt-1">Terakhir diubah: {{ $inventaris->updated_at->format('d M Y, H:i')
                    }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        <form method="POST" action="{{ route('admin.sekretariat.inventaris.update', $inventaris->id) }}"
            class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori"
                        ...
                        <option value="elektronik" {{ old('kategori', $inventaris->kategori) == 'elektronik' ?
                            'selected' : '' }}>Elektronik</option>
                        <option value="furniture" {{ old('kategori', $inventaris->kategori) == 'furniture' ? 'selected'
                            : '' }}>Furniture</option>
                        <option value="kendaraan" {{ old('kategori', $inventaris->kategori) == 'kendaraan' ? 'selected'
                            : '' }}>Kendaraan</option>
                        <option value="peralatan" {{ old('kategori', $inventaris->kategori) == 'peralatan' ? 'selected'
                            : '' }}>Peralatan</option>
                    </select>
                    @error('kategori')
                    <p class="text-sm text-red-600 mt-2 flex items-center gap-1">
                ...
                <div>
                    ...
                    <input type="number" id="jumlah" name="jumlah" value="{{ old('jumlah', $inventaris->jumlah) }}"
                        min="1"
                ...
                <div>
                    ...
                    <select id="kondisi" name="kondisi"
                        ...
                        <option value="baik" {{ old('kondisi', $inventaris->kondisi) == 'baik' ? 'selected' : '' }}>Baik
                        </option>
                        <option value="rusak" {{ old('kondisi', $inventaris->kondisi) == 'rusak' ? 'selected' : ''
                            }}>Rusak</option>
                        <option value="perlu_perbaikan" {{ old('kondisi', $inventaris->kondisi) == 'perlu_perbaikan' ?
                            'selected' : '' }}>Perlu Perbaikan</option>
                    </select>
                    @error('kondisi')
                    <p class="text-sm text-red-600 mt-2 flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    ...
                    <input type="text" id="lokasi" name="lokasi" value="{{ old('lokasi', $inventaris->lokasi) }}"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="harga_perolehan" name="harga_perolehan"
                            value="{{ old('harga_perolehan', $inventaris->harga_perolehan) }}" min="0" step="0.01"
                ...
                <div>
                    ...
                    <input type="text" id="sumber_perolehan" name="sumber_perolehan"
                        value="{{ old('sumber_perolehan', $inventaris->sumber_perolehan) }}"
            ...
            <div>
                ...
                <textarea id="keterangan" name="keterangan" rows="2"
                    ...
                    placeholder="Keterangan tambahan jika diperlukan">{{ old('keterangan', $inventaris->keterangan) }}</textarea>
                @error('keterangan')
                <p class="text-sm text-red-600 mt-2 flex items-center gap-1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
